...
vẫn giữ các thay đổi công cụ này.** `main` phản ánh Blender (cộng thêm một script
khởi tạo `nmake_locale.sh`); phần việc
...
# Cập nhật main với thay đổi của Blender (main cũng chứa nmake_locale.sh,
# nên dùng merge thường, không dùng --ff-only — vẫn sạch vì Blender không
# bao giờ động đến script khởi tạo này)
git checkout main
...
mỗi thứ Hai (và theo yêu cầu qua tab Actions). Nó cập nhật `main` (hợp nhất thay
đổi của Blender), thử hợp nhất `upstream/main` vào nhánh fork, rồi hoặc là:
