...
In order to allow a smooth transition process, the so-called legacy add-ons will continue to be supported by Blender. They can be installed via :doc:`Install legacy Add-on </editors/preferences/extensions>` button in the User Preferences.
...
.. note::
  ...
  For testing it is import to :doc:`install the extension from disk </editors/preferences/extensions>` and check if everything is working well. This will get you as close to the final experience as possible.

Extensions and Namespace
========================

The legacy add-ons would use their module name to access the preferences. This could lead to a name clash when extensions with the same name (from different repositories) would be installed.
...
Add-ons can define their own preferences, and can access them using the complete module name. This is done by using ``__package__``.
...
Importing packages within the add-on module need to use relative paths. This is a standard Python feature and only appliable for add-ons that have multiple folders.
...
Extensions are supposed to be self-contained, and as such must come with all its depedencies. When relying on external modules they should be bundled together by using `wheels <https://pythonwheels.com/>`__.
...
Luckily there is an alternative way of loading wheels that doesn't affect sys.modules or sys.path. This way an add-on can load its own version of an external library from its bundled wheel file, without no other add-on having access to it.
